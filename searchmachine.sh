...
function helpPanel(){
  ...
  echo -e "\t${purpleColour}d)${endColour}${grayColour} Buscar por dificultad de la maquinas${endColour}"
  ...
  echo -e "\t${purpleColour}h)${endColour} Mostar este panel de ayuda${endColour}\n" 
}
...
function updateFiles(){
  ...
  if [ ! -f bundle.js ]; then
    ...
    if [ "$md5_tem_value" == "$md5_original_value" ]; then
      echo -e "\n${yellowColour}[+]${endColour}${yellowColour} No se han encontrado actualizaciones, esta todo al dia ;)${endColour}"
      rm bundle_temp.js
    else
      echo -e "\n${yellowColour}[+]${endColour}${grayColour} Se han encontrado actualizaciones disponibles :3${endColour}"
      sleep 2
...
function getMachinesDifficulty(){
  ...
  if [ "$results_check" ]; then
    echo -e "\n${blueColour}[+]${endColour}${grayColour} Respresentando las maquinas que poseen un nivel de dificultad $difficulty${endColour}${grayColour}:${endColour}\n"
...
function getOSDifficultyMachines(){
  ...
  if [ "$check_results" ]; then
    ...
    echo -e "\n${redColour}[!] Se ha indicad una dificultad o sistema operativo incorrectos${endColour}\n"
  fi
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    echo -e "\n${blueColour}[+] A continucion se representaran las maquinas donde se ve la tecnica ${endColour}${redColour} $skill${endColour}\n"
    cat bundle.js | grep "skills: " -B 6 | grep "$skill" -i -B 6 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column 
  else
    echo -e "\n${redColour}[!] No se ha encontrado ninguna maquina con esta skill${endColour}\n"
  fi 
